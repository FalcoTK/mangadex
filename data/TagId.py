class TagAdvance:
    def __init__(self):
        self.dataID = {
            ...
            "Boy Love": "5920b825-4181-4a17-beeb-9918b0ff7a30",
    ...
    async def FilterTagsID(self, TagID):
        tagtosearch = []
        ...
        for name in TagID:
            ...
            # Search for the tag in offline data
            for tag_name, tag_id in self.dataID.items():
                if tag_name.strip().lower() == normalized_name:
                    tagtosearch.append(tag_id)
                    found = True
                    break

            if not found:
                return f"Tag '{name}' not found, did you write it correctly?"

        return tagtosearch

    async def ShortTypeID(self, idRequest):
        if idRequest in self.ShortType:
            return self.ShortType[idRequest]
        return f"Short Type '{idRequest}' not found, did you write it correctly?"
